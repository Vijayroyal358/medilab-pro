  ...
  const fetchData = async (t: string) => {
    ...
    try {
      const [s, l, st, a] = await Promise.all([
        fetch(`${API}/superadmin/dashboard-stats`, { headers: h(t) }),
        fetch(`${API}/superadmin/labs`, { headers: h(t) }),
        fetch(`${API}/superadmin/recent-staff`, { headers: h(t) }),
        fetch(`${API}/superadmin/recent-activity`, { headers: h(t) }),
      ]);
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <input type="text" placeholder="Search..." className="pl-9 pr-12 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#00A770]/20 focus:border-[#00A770] w-64" />
              <div className="absolute right-2 top-1/2 -translate-y-1/2 px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[10px] font-bold text-slate-400">⌘K</div>
            </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                      <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
                        ...
                        <button className="text-xs font-bold text-slate-500 hover:text-[#00A770] px-3 py-1.5 border border-slate-200 rounded-lg transition-all">View all</button>
                      </div>
                          ...
                          <tbody className="divide-y divide-slate-50">
                            {labs.slice(0, 5).map(lab => (
                              <tr key={lab.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center font-bold">{lab.staff_count}</td>
                                ...
                                <td className="px-6 py-4 text-right">
                                  <button className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><MoreVertical className="w-4 h-4" /></button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm max-w-2xl">
                  ...
                  <div className="border-t border-slate-100 pt-6">
                    ...
                    <form
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (res.ok) { alert("Password updated successfully!"); form.reset(); }
                    ...
                    >
                      ...
                      <button type="submit" className="px-6 py-2.5 bg-[#00A770] hover:bg-[#009060] text-white font-extrabold text-xs rounded-xl shadow-md transition-all uppercase tracking-wider">
                        Update Password
                      </button>
                    </form>
                  </div>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
        active ? "bg-emerald-50 text-[#00A770] font-extrabold" : "text-slate-500 font-semibold hover:bg-slate-50 hover:text-slate-700"
      }`}
    >
      {icon}
